 ...
 *
 * If a `struct blob_info' blob has
 * (blob->resource_location == RESOURCE_IN_WIM), then blob->wim_res_spec points to
 ...
 * and WIM resources.  However, WIM resources with the WIM_RESHDR_FLAG_SOLID
 * flag set may actually contain multiple streams compressed together.
 */

...
static int
read_wim_stream_prefix(const struct blob *blob, u64 size,
		       consume_data_callback_t cb, void *cb_ctx)
{
...
typedef int (*read_stream_prefix_handler_t)(const struct blob *blob,
					    u64 size,
 ...
 */
static int
read_stream_prefix(const struct blob *blob, u64 size,
		   consume_data_callback_t cb, void *cb_ctx)
{
...
int
read_full_stream_into_buf(const struct blob *blob, void *_buf)
{
	u8 *buf = _buf;
...
wim_resource_spec_to_data(struct wim_resource_spec *rspec, void **buf_ret)
{
	...
	lte_bind_wim_resource_spec(blob, rspec);
...
struct streamifier_context {
	...
	struct blob *final_stream;
	size_t list_head_offset;
};

static struct blob *
next_stream(struct blob *blob, size_t list_head_offset)
{
...
static int
read_streams_in_solid_resource(struct blob *first_stream,
 ...
 *
 * @stream_list
 *	List of streams (represented as `struct blob's) to
 *	read.
 * @list_head_offset
 *	Offset of the `struct list_head' within each `struct
 *	blob' that makes up the @stream_list.
 * @cbs
	     ...
	     cur = next, next = cur->next)
	{
		    ...
		    blob->size != blob->rspec->uncompressed_size)
		{
			...
			if (stream_count > 1) {
				...
				next = next2;
				ret = read_streams_in_solid_resource(blob, lte_last,
								     stream_count,
								     list_head_offset,
								     sink_cbs);
				if (ret)

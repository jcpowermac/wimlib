...
new_blob_table(size_t capacity)
{
	...
	table = MALLOC(sizeof(struct wim_blob_table));
	if (table == NULL)
		goto oom;
...
new_blob_info(void)
{
	...
	blob = CALLOC(1, sizeof(struct blob_info));
	if (blob == NULL)
		return NULL;
...
struct blob_info *
clone_blob_table_entry(const struct blob_info *old)
{
	struct blob_info *new;

	new = memdup(old, sizeof(struct blob_info));
	if (new == NULL)
		return NULL;
...
static void
blob_table_insert_raw(struct wim_blob_table *table,
			struct blob_info *blob)
{
...
static void
enlarge_blob_table(struct wim_blob_table *table)
{
...
cmp_streams_by_sequential_order(const void *p1, const void *p2)
{
	...
	case RESOURCE_IN_WIM:
		...
		if (blob1->rspec->offset_in_wim != blob2->rspec->offset_in_wim)
			return cmp_u64(blob1->rspec->offset_in_wim,
				       blob2->rspec->offset_in_wim);
		 ...
		 int (*compar)(const void *, const void*))
{
	...
	for (i = 0; i < num_streams; i++) {
		array[i] = (struct blob_info*)((u8*)cur -
							    list_head_offset);
		cur = cur->next;
	}
...
static int
add_lte_to_array(struct blob_info *blob,
		 void *_pp)
{
	struct blob_info ***pp = _pp;
	*(*pp)++ = blob;
	return 0;
}
...
for_blob_table_entry_pos_sorted(struct wim_blob_table *table,
				  ...
				  void *arg)
{
	struct blob_info **blob_array, **p;
	size_t num_streams = table->num_entries;
	int ret;

	blob_array = MALLOC(num_streams * sizeof(blob_array[0]));
	if (!blob_array)
		return WIMLIB_ERR_NOMEM;
	p = blob_array;
	for_blob_table_entry(table, add_lte_to_array, &p);
...
static size_t
count_solid_resources(const struct blob_info_disk *entries, size_t max)
{
	size_t count = 0;
	do {
...
do_load_solid_info(WIMStruct *wim, struct wim_resource_spec **rspecs,
		   size_t num_rspecs,
		   const struct blob_info_disk *entries)
{
...
/* Given a 'struct blob_info' allocated for a stream entry with the
...
static void
free_solid_rspec_info(struct wim_resource_spec **rspecs, size_t num_rspecs)
{
	if (rspecs) {
		for (size_t i = 0; i < num_rspecs; i++)
...
finish_solid_rspecs(struct wim_resource_spec **rspecs, size_t num_rspecs)
{
	...
	free_solid_rspec_info(rspecs, num_rspecs);
	return ret;
}
...
read_wim_blob_table(WIMStruct *wim)
{
	...
	size_t num_entries;
	void *buf = NULL;
	struct wim_blob_table *table = NULL;
	struct blob_info *cur_entry = NULL;
	 ...
	 * blob_info's) from the raw lookup table buffer.  Each of
	...
	for (size_t i = 0; i < num_entries; i++) {
		const struct blob_info_disk *disk_entry =
			&((const struct blob_info_disk*)buf)[i];
		struct wim_reshdr reshdr;
		u16 part_number;
...
static void
put_blob_info(struct blob_info_disk *disk_entry,
					...
					int write_resource_flags)
{
	size_t table_size;
	struct blob_info *blob;
	struct blob_info_disk *table_buf;
	struct blob_info_disk *table_buf_ptr;
	int ret;
	...
	list_for_each_entry(blob, stream_list, blob_table_list) {
		table_size += sizeof(struct blob_info_disk);
			    ...
			    struct wim_blob_table *blob_table)
{
	...
	existing_blob = lookup_stream(blob_table, hash);
	if (existing_blob) {
		     ...
		     struct blob_info **blob_ret)
{
	...
	duplicate_blob = lookup_stream(blob_table, blob->hash);
	list_del(&blob->unhashed_list);
	if (duplicate_blob) {
			    ...
			    void *user_ctx)
{
	...
	return for_blob_table_entry(wim->blob_table, do_iterate_blob, &ctx);
}

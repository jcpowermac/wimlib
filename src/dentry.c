 ...
 * with WIMGAPI, nor is there any way for it to be read or written on Windows
 * when the process does not have access to the file encryption key.
 */
static void
assign_stream_types_encrypted(struct wim_inode *inode)
{
	bool found_efsdata = false;
	for (unsigned i = 0; i < inode->i_num_streams; i++) {
		struct wim_inode_stream *strm = &inode->i_streams[i];
		if (!found_efsdata && !stream_is_named(strm) &&

...
dentry_out_total_length(const struct wim_dentry *dentry)
{
	...
	if (!(inode->i_attributes & FILE_ATTRIBUTE_ENCRYPTED)) {
		...
		for (unsigned i = 0; i < inode->i_num_streams; i++) {
			...
			} else if (strm->stream_type == STREAM_TYPE_DATA &&
...
/*
 * Set the type of each stream read for a file with FILE_ATTRIBUTE_ENCRYPTED
 * set.
 *
 * For this type of file, all the data streams of the file should have been
 * packed into a single stream in a format generated by ReadEncryptedFileRaw()
 * on Windows.  We assign this stream type STREAM_TYPE_EFSRPC_RAW_DATA.
 *
 * Encrypted files can't be a reparse point stream.  On-disk there can be, but
 * as far as I know it can't be stored in the WIM format in a way that's
 * compatible with WIMGAPI, nor is there any way for it to be restored on
...
assign_stream_types_encrypted(struct wim_inode *inode)
{
	...
	for (unsigned i = 0; i < inode->i_num_streams; i++) {
		...
		if (!found_efsrpc_stream && !stream_is_named(strm) &&
		    !is_zero_hash(strm->_stream_hash)) {
...
/*
 * Set the type of each stream read for a file without FILE_ATTRIBUTE_ENCRYPTED
 * set.
 *
 * There will be an unnamed data stream, zero or more named data stream, and if
 * FILE_ATTRIBUTE_REPARSE_POINT is set there should be a reparse point stream.
 */
static void
...
write_dentry(const struct wim_dentry * restrict dentry, u8 * restrict p)
{
	...
	if (inode->i_attributes & FILE_ATTRIBUTE_ENCRYPTED) {
		const struct wim_inode_stream *efsrpc_stream;
		const u8 *efsrpc_hash;

		efsrpc_stream = inode_get_stream(inode, STREAM_TYPE_EFSRPC_RAW_DATA,
						 NO_STREAM_NAME);
		if (efsrpc_stream)
			efsrpc_hash = stream_hash(efsrpc_stream);
		else
			efsrpc_hash = zero_hash;

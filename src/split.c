...
static int
add_stream_to_swm(struct blob_info *blob, void *_swm_info)
{
	struct swm_info *swm_info = _swm_info;
	u64 stream_size;

	if (blob_is_partial(blob)) {
	...
	if (!(blob->b_flags & WIM_RESHDR_FLAG_METADATA)) {
		list_add_tail(&blob->b_write_blobs_list,
	     ...
	     u64 part_size, int write_flags)
{
	...
	ret = for_blob_info_pos_sorted(wim->blob_table,
						add_stream_to_swm,
						&swm_info);
	if (ret)
		goto out_free_swm_info;

 ...
 */
static int
stream_filtered(const struct blob_info *blob,
...
static bool
stream_hard_filtered(const struct blob_info *blob,
		     struct filter_context *ctx)
{
	return stream_filtered(blob, ctx) < 0;
}
...
static bool
can_raw_copy(const struct blob_info *blob,
...
static void
stream_set_out_reshdr_for_reuse(struct blob_info *blob)
{
	const struct wim_resource_spec *rspec;
...
static int
write_pwm_stream_header(const struct blob_info *blob,
...
static inline bool
is_file_stream(const struct blob_info *blob)
{
...
static int
do_done_with_stream(struct blob_info *blob,
...
write_blob_begin_read(struct blob_info *blob, void *_ctx)
{
	 ...
	 * still provide the data again to write_blob_process_chunk().  This
	...
	if (ctx->blob_table != NULL && blob->b_unhashed && !blob->unique_size) {
		...
		if (blob_new != blob) {
			...
			if (blob_new->will_be_in_output_wim ||
			    stream_filtered(blob_new, ctx->filter_ctx))
			{
				...
				ret = do_write_blobs_progress(&ctx->progress_data,
								blob->b_size,
								1, true);
				list_del(&blob->b_write_blobs_list);
				...
				list_replace(&blob->b_write_blobs_list,
					     &blob_new->b_write_blobs_list);
				list_replace(&blob->blob_table_list,
					     &blob_new->blob_table_list);
				blob->will_be_in_output_wim = 0;
				blob_new->out_refcnt = blob->out_refcnt;
				blob_new->will_be_in_output_wim = 1;
				blob_new->may_send_done_with_file = 0;
				blob = blob_new;
			}
		}
	}
	list_move_tail(&blob->b_write_blobs_list, &ctx->pending_streams);
	return 0;
}
...
static int
write_blob_uncompressed(struct blob_info *blob,
			  struct filedes *out_fd)
{
	int ret;
 ...
 * and re-written as uncompressed.  blob->out_reshdr must be filled in from the
				   ...
				   const struct blob_info *blob)
{
	 ...
	 * single-chunk resources in maybe_rewrite_blob_uncompressed()
	    ...
	    size_t csize, size_t usize)
{
	...
	blob = list_entry(ctx->pending_streams.next,
			 struct blob_info, b_write_blobs_list);
	    ...
	    !(ctx->write_resource_flags & WRITE_RESOURCE_FLAG_SOLID))
	{
		...
		if (ctx->write_resource_flags & WRITE_RESOURCE_FLAG_PIPABLE) {
			...
			ret = write_pwm_stream_header(blob, ctx->out_fd,
	...
	if (ctx->write_resource_flags & WRITE_RESOURCE_FLAG_SOLID) {
		...
		while (blob && ctx->cur_write_blob_offset >= blob->b_size) {

			ctx->cur_write_blob_offset -= blob->b_size;

			if (ctx->cur_write_blob_offset)
				next_blob = list_entry(blob->b_write_blobs_list.next,
						      struct blob_info,
						      b_write_blobs_list);
			else
				next_blob = NULL;
			...
			if (ret)
				return ret;
			list_move_tail(&blob->b_write_blobs_list, &ctx->solid_streams);
		 ...
		 * stream.  */
		if (ctx->cur_write_blob_offset == blob->b_size) {
	...
	return do_write_blobs_progress(&ctx->progress_data,
...
static int
submit_chunk_for_compression(struct write_blobs_ctx *ctx,
...
static int
write_blob_end_read(struct blob_info *blob, int status, void *_ctx)
{
	struct write_blobs_ctx *ctx = _ctx;
	...
	if (!blob->will_be_in_output_wim) {
		/* The 'blob' stream was a duplicate.  Now that its data has
			  ...
			  struct write_blobs_ctx *ctx)
{
	...
	list_for_each_entry(blob, blob_list, b_write_blobs_list) {
		num_streams++;
...
/* Find streams in @blob_list that can be copied to the output WIM in raw form
 * rather than compressed.  Delete these streams from @blob_list and move them
 * to @raw_copy_streams.  Return the total uncompressed size of the streams that
 * need to be compressed.  */
static u64
find_raw_copy_streams(struct list_head *blob_list,
		      int write_resource_flags,
		      int out_ctype,
		      u32 out_chunk_size,
		      struct list_head *raw_copy_streams)
{
	...
	list_for_each_entry_safe(blob, tmp, blob_list, b_write_blobs_list) {
		if (blob->resource_location == RESOURCE_IN_WIM &&
		    blob->rspec->raw_copy_ok)
		{
			list_move_tail(&blob->b_write_blobs_list,
				       raw_copy_streams);
			...
			struct filedes *out_fd)
{
	...
	list_for_each_entry(blob, &in_rspec->blob_list, rspec_node) {
		if (blob->will_be_in_output_wim) {
			stream_set_out_reshdr_for_reuse(blob);
			 ...
			 struct write_blobs_progress_data *progress_data)
{
	...
	list_for_each_entry(blob, raw_copy_streams, b_write_blobs_list)
		blob->rspec->raw_copy_ok = 1;
	...
	list_for_each_entry(blob, raw_copy_streams, b_write_blobs_list) {
		...
		ret = do_write_blobs_progress(progress_data, blob->b_size,
						1, false);
		if (ret)
			return ret;
	}
...
static void
remove_zero_length_streams(struct list_head *blob_list)
{
	struct blob_info *blob, *tmp;
 ...
 *	blob_info's linked by the 'b_write_blobs_list' member.
 *
 * @out_fd
 ...
 *	can be NULL.
 *
 * This function will write the streams in @blob_list to resources in
 ...
 * cases, the @out_reshdr of the `struct blob_info' for each stream
 ...
 * @will_be_in_output_wim member be set to 1 on all streams in @blob_list as
 * well as any other streams not in @blob_list that will be in the output WIM
 * file, but set to 0 on any other streams in the output WIM's lookup table or
 * sharing a solid resource with a stream in @blob_list.  Still furthermore,
 * if on-the-fly deduplication of streams is possible, then all streams in
 * @blob_list must also be linked by @blob_table_list along with any other
 ...
 * @blob_list that has @unhashed set but not @unique_size set is checksummed
 ...
 * @blob_list, its reference count transfered to the pre-existing duplicate
 * stream, its memory freed, and will not be written.  Alternatively, if a
 * stream in @blob_list is a duplicate with any stream in @blob_table that
		  ...
		  void *progctx)
{
	...
	remove_zero_length_streams(blob_list);
	...
	/* Pre-sorting the streams is required for compute_blob_list_stats().
	 * Afterwards, read_blob_list() need not sort them again.  */
	...
	ctx.num_bytes_to_compress = find_raw_copy_streams(blob_list,
							  write_resource_flags,
							  out_ctype,
							  out_chunk_size,
							  &raw_copy_streams);
	...
	struct read_blob_list_callbacks cbs = {
		.begin_stream		= write_blob_begin_read,
		.begin_stream_ctx	= &ctx,
		.consume_chunk		= write_blob_process_chunk,
		.consume_chunk_ctx	= &ctx,
		.end_stream		= write_blob_end_read,
		.end_stream_ctx		= &ctx,
	};

	ret = read_blob_list(blob_list,
			       offsetof(struct blob_info, b_write_blobs_list),
			       &cbs,
	...
	if (write_resource_flags & WRITE_RESOURCE_FLAG_SOLID) {
		...
		offset_in_res = 0;
		list_for_each_entry(blob, &ctx.solid_streams, b_write_blobs_list) {
			blob->out_reshdr.size_in_wim = blob->b_size;
			blob->out_reshdr.flags = filter_resource_flags(blob->b_flags);
...
static int
is_stream_in_solid_resource(struct blob_info *blob, void *_ignore)
{
	return blob_is_partial(blob);
}

static bool
wim_has_solid_resources(WIMStruct *wim)
{
	return for_blob_info(wim->blob_table,
...
static int
write_wim_resource(struct blob_info *blob,
		   struct filedes *out_fd,
		   int out_ctype,
		   u32 out_chunk_size,
		   int write_resource_flags)
{
	LIST_HEAD(blob_list);
	list_add(&blob->b_write_blobs_list, &blob_list);
...
static int
stream_size_table_insert(struct blob_info *blob, void *_tab)
{
...
static void
reference_stream_for_write(struct blob_info *blob,
			   struct list_head *blob_list, u32 nref)
{
	if (!blob->will_be_in_output_wim) {
		blob->out_refcnt = 0;
		list_add_tail(&blob->b_write_blobs_list, blob_list);
		blob->will_be_in_output_wim = 1;
	}
...
static int
fully_reference_stream_for_write(struct blob_info *blob,
				 void *_blob_list)
{
	struct list_head *blob_list = _blob_list;
	blob->will_be_in_output_wim = 0;
	reference_stream_for_write(blob, blob_list, blob->refcnt);
	return 0;
}
				...
				struct list_head *blob_list)
{
	...
	for (i = 0; i <= inode->i_num_ads; i++) {
		blob = inode_get_blob_for_stream(inode, i, table);
		if (blob)
			reference_stream_for_write(blob, blob_list,
						   inode->i_nlink);
...
static int
do_stream_set_not_in_output_wim(struct blob_info *blob, void *_ignore)
{
	blob->will_be_in_output_wim = 0;
	return 0;
}
...
static int
insert_other_if_hard_filtered(struct blob_info *blob, void *_ctx)
{
	struct insert_other_if_hard_filtered_ctx *ctx = _ctx;

	if (!blob->will_be_in_output_wim &&
	    stream_hard_filtered(blob, ctx->filter_ctx))
		stream_size_table_insert(blob, ctx->tab);
	return 0;
}

static int
determine_stream_size_uniquity(struct list_head *blob_list,
			       ...
			       struct filter_context *filter_ctx)
{
	...
	if (may_hard_filter_streams(filter_ctx)) {
		...
		for_blob_info(lt, insert_other_if_hard_filtered, &ctx);
	}

	list_for_each_entry(blob, blob_list, b_write_blobs_list)
		stream_size_table_insert(blob, &tab);
			     ...
			     struct filter_context *filter_ctx)
{
	...
	list_for_each_entry_safe(blob, tmp,
				 blob_list, b_write_blobs_list)
	{
		int status = stream_filtered(blob, filter_ctx);
 ...
 *	@wim, shall be returned in neither @blob_list_ret nor
 *	@blob_table_list_ret.
 *
 * @blob_list_ret
 *	List of streams, linked by b_write_blobs_list, that need to be written
 ...
 *	superset of the streams in @blob_list_ret.
 *
 ...
 * @blob_list_ret that have unique size among all stream entries in
 * @blob_list_ret and among all stream entries in the lookup table of @wim
			      ...
			      struct filter_context *filter_ctx_ret)
{
	...
	list_for_each_entry(blob, blob_list_ret, b_write_blobs_list)
		list_add_tail(&blob->blob_table_list, blob_table_list_ret);

	ret = determine_stream_size_uniquity(blob_list_ret, wim->blob_table,
					     filter_ctx_ret);
	if (ret)
		return ret;
...
static int
write_wim_blobs(WIMStruct *wim, int image, int write_flags,
		  unsigned num_threads,
	...
	for (int i = start_image; i <= end_image; i++) {
		...
		if (imd->modified) {
			...
			DEBUG("Image %u was not modified; re-using existing "
			      "metadata resource.", i);
			stream_set_out_reshdr_for_reuse(imd->metadata_blob);
			ret = 0;
		} else {
...
cmp_streams_by_out_rspec(const void *p1, const void *p2)
{
	...
	blob1 = *(const struct blob_info**)p1;
	blob2 = *(const struct blob_info**)p2;
		       ...
		       struct list_head *blob_table_list)
{
	...
	ret = sort_blob_list(blob_table_list,
			       offsetof(struct blob_info, blob_table_list),
 ...
 *   blob_info_disk' that only contains the SHA1 message digest,
		  ...
		  struct list_head *blob_table_list_ret)
{
	...
	return write_wim_blobs(wim, image, write_flags, num_threads,
				 blob_list_override, blob_table_list_ret);
	       ...
	       const u8 *guid)
{
	...
	DEBUG("Stream list:       %s", (blob_list_override ? "specified" : "autodetect"));
	...
	if (!(write_flags & WIMLIB_WRITE_FLAG_PIPABLE)) {
		...
		ret = write_wim_blobs(wim, image, write_flags, num_threads,
					blob_list_override,
					&blob_table_list);
		if (ret)
			goto out_restore_hdr;
...
static int
check_resource_offset(struct blob_info *blob, void *_wim)
{
	const WIMStruct *wim = _wim;
	off_t end_offset = *(const off_t*)wim->private;

	if (blob->resource_location == RESOURCE_IN_WIM && blob->rspec->wim == wim &&
...
check_resource_offsets(WIMStruct *wim, off_t end_offset)
{
	...
	ret = for_blob_info(wim->blob_table, check_resource_offset, wim);
	if (ret)
		return ret;

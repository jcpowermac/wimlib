...
static void
lzms_sort_symbols_by_frequency(u32 * const restrict A,
...
static void
lzms_generate_lengths(u32 * const restrict A,
		      unsigned * const restrict len_counts,
		      u8 * const restrict lens)
{
	unsigned i = 0;
	for (unsigned len = LZMS_MAX_CODEWORD_LEN; len >= 1; len--) {
...
static void
lzms_sort_symbols_by_codeword_length(u32 * const restrict A,
				     u8 * const restrict lens,
				     unsigned * const restrict len_counts,
				     const unsigned num_syms)
{
	unsigned offsets[LZMS_MAX_CODEWORD_LEN + 1];

	offsets[1] = 0;
	for (unsigned len = 1; len < LZMS_MAX_CODEWORD_LEN; len++)
		offsets[len + 1] = offsets[len] + len_counts[len];

	for (unsigned sym = 0; sym < num_syms; sym++)
		A[offsets[lens[sym]]++] = sym;
}
...
lzms_fill_decode_table(u16 * const restrict decode_table,
		       ...
		       const u32 * const restrict sorted_syms)
{
...
lzms_rebuild_huffman_code(struct lzms_huffman_rebuild_info *info)
{
	...
	lzms_sort_symbols_by_frequency(A, info->freqs, info->num_syms);

	lzms_build_huffman_tree(A, info->num_syms);

	lzms_generate_length_counts(A, len_counts, info->num_syms);

	lzms_generate_lengths(A, len_counts, info->lens);

	lzms_sort_symbols_by_codeword_length(A, info->lens, len_counts, info->num_syms);
